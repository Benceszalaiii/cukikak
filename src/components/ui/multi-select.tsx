...
interface MultiSelectorTriggerProps extends React.HTMLAttributes<HTMLDivElement> {
  nameValueMapping?: { name: string, value: string }[];
}

const MultiSelectorTrigger = forwardRef<
  HTMLDivElement,
  MultiSelectorTriggerProps
>(({ className, children, nameValueMapping, ...props }, ref) => {
    ...
    >
      {value.map((item, index) => (
        <Badge
          ...
          variant={"secondary"}
        >
          <span className="text-xs">{nameValueMapping?.find((x)=> x.value == item)?.name || "No name"}</span>
          <button
            aria-label={`${item} eltávolítása`}
            aria-roledescription="eltavolitas gomb"
            type="button"
            onMouseDown={mousePreventDefault}
            onClick={() => onValueChange(item)}
          >
            <span className="sr-only">{item} eltávolítása</span>
